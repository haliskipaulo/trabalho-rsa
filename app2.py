...
def share_keys():
    ...
    N_app1 = data['N']
    E_app1 = data['E']
    print(f"Chaves recebidas da outra aplicação: N={N_app1}, E={E_app1}")
...
def send_message():
    ...
    try:
        ...
        response = req.post(f"{URL_APP1}/receive_message", json=payload)
...
def receive_message():
    ...
    try:
        decrypted_message = decrypt(encrypted_message_received, N_app2, D)
...
def initiate_key_exchange():
    ...
    payload = {"N": N_app2, "E": E_app2}
    try:
        print(f"Enviando minhas chaves N e E para {URL_APP1}/share_keys...")
        response = req.post(f"{URL_APP1}/share_keys", json=payload)
        response.raise_for_status()
...
def attemp_key_exchange():
    ...
    for num in range(max_tries):
        try:
            response = req.post(f"{URL_APP1}/share_keys", json=payload, timeout=10)
            response.raise_for_status()
            return
...
if __name__ == '__main__':
    ...
    print(f"Minhas chaves: N={N_app2}, E={E_app2}, D={D}")
